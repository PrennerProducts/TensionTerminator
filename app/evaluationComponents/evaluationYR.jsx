...
const evaluationYR = () => {
  ...
  const [exercise, setExercise] = useState(params.exercise); // 0: Yaw, 1: Roll
  ...
  const startNextEvaluation = async () => {
      if (exercise === 0) {
          console.log("Begin next exercise...");
          setExercise(1);
          startEvaluation();
      }
      else console.log("Evaluation finished!"); // Exit?
  };
      ...
      {evaluationStarted && (
        <View style={styles.textContainer}>
          ...
          <Text style={styles.maxValues}>
            Max Right: {maxR.toFixed(2)}°, Max Left: {maxL.toFixed(2)}°
          </Text>
        </View>
      )}
    </View>
